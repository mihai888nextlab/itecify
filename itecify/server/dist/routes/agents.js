...
import { ensureContainer, terminalRead, terminalWrite, terminalDelete, terminalList } from '../services/dockerService.js';
...
const aiRoutes = async (fastify) => {
    ...
    async function findFileByPath(projectId, filePath) {
    ...
    async function getParentForPath(projectId, filePath) {
        const parts = filePath.split('/').filter(p => p);
        if (parts.length <= 1)
    ...
    async function executeFileCommand(projectId, command) {
        try {
            switch (command.type) {
                case 'READ': {
    ...
    fastify.post('/agents/execute', async (request, reply) => {
        ...
        if (!apiKey)
            return reply.code(500).send({ success: false, error: 'AI service not configured' });
        try {
            ...
            if (fileOperations.length > 0) {
                ...
                if (successOps.length > 0) {
                    ...
                    for (const op of successOps)
                        summary += `- ✅ ${op.type} ${op.path}: ${op.message}\n`;
                }
